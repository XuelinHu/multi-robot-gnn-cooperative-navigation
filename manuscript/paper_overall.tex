...
本文的主要贡献包括：
\begin{enumerate}
  \item 构建一个可复现的多机器人路径规划实验框架，统一实现数据生成、基线、训练、评估和可视化；
  \item 设计基于消息传递的 GNN 局部策略，并将节点特征、动态邻接关系和安全过滤层纳入统一控制流程；
  \item 在四类地图、5--40 个机器人规模和 30 个未见测试种子上，对 A*、ORCA-style、RVO-style、MLP、GNN、Attention-GNN 和 Graph Transformer 进行比较，并开展安全层、通信半径和图结构消融；
  \item 将控制器接入 Wheeltec Gazebo/RViz2 多机器人环境，验证命名空间、里程计、激光、速度控制和可视化话题链路。
\end{enumerate}
...
\section{方法}
\subsection{总体架构}
图~\ref{fig:architecture} 展示了从多机器人观测、动态图构建到动作执行的完整闭环。每个机器人对应一个图节点，边表示通信范围内的邻居关系。编码器和消息传递层使用共享参数，因而可以处理不同数量的机器人。

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{gnn_architecture_ai_v3.png}
\caption{基于动态图消息传递的多机器人协同路径规划架构。}
